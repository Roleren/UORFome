...
#' @param fac a factor that is a number
an <- function(fac){
...
#' @param reset reset cluster, use only if already existing
#' @param outfile log outputs
#' @export
...
updateORFik <- function(branch = "master", user = "Roleren")  {
  system("rm -r /Home/ii/hakontj/R/x86_64-redhat-linux-gnu-library/R-3.6.0/library/00LOCK-ORFik")
  devtools::install_github(paste0(user, "/ORFik"), ref = branch)
}

#' Export GRanges object as 2 wig files
#'
#' Split into +/- strand
#' @param gr GRanges object
#' @param outputPrefix prefix and folder for output,
#' will add forward and reverse
#' @export
