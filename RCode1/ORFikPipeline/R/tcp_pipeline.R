...
#' @inheritParams tcpHeatMap_single
#' @export
...
#' Heatmap of single library
#' @inheritParams ORFik::transcriptWindow
#' @export
...
regionBarPlotAll <- function(region, tx, df, outdir, format = ".png",
                          ...
                          proportion = TRUE, location = "TIS", acLen = NULL) {
  up <- upstream; down <- downstream
  dfl <- df
  if(!is(dfl, "list")) dfl <- list(dfl)
  for (df in dfl) {
    ...
    for (i in varNames) { # For each stage
      for (score in scores) {
        for (s in 1:length(shifting)) {
          ...
          if (!is.null(acLen)) reads <- reads[readWidths(reads) %in% acLen]
...
#' @param trna the fasta tRNA from trnaScan ?
#' @return a data.frame of imet and met content
#' @export
...
#' @export
removeBadTxByRegion <- function(tx, reads, upstream = NULL, downstream = NULL, median_multiplier = "0.99",
...
#' Split metacoverage by a splitList
