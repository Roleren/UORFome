# INFO: 
# Here are some of the box plot figures in RCP-seq article
...
###################################################################################################
# Old pallets
# pallet_4n <- c("#A87225", "#146DA8", "#F5B04E", "#4EB3F5")
# pallet_5 <- c("#A87225", "#FFC26B", "#F5B04E", "#4EB3F5", "#146DA8")
# pallet_4 <- c("#A87225", "#F5B04E", "#4EB3F5", "#146DA8")
# pallet_3 <- c("#A87225", "#F5B04E", "#146DA8")
# pallet_2 <- c("#A87225", "#146DA8")

new_pallet_4 <- c("#E495A5","#ABB065","#39BEB1","#ACA4E2")
# #E495A5"), Not TOP C starting (#39BEB1"), Not TOP other (#ACA4E2"
new_pallet_3 <- c("#39BEB1", "#ACA4E2", "#E495A5")

###################################################################################################
# Figure 2 D & E
# New RNA seq 
combined.dat.ranked <-  readRDS("/export/valenfs/projects/Hakon/RCP_SEQ/adam_matrix_combined_nonfiltered_new_RNAseq.rds")
...
se1 <- ggplot(data=data.subset.by.rna.melt, aes((value), colour = TSS_start_sequence)) +
  stat_ecdf() +
  #scale_x_log10(limits=c(0.08,25)) +
  scale_x_log10() + 
  coord_cartesian(xlim=c(0.1,10)) + 
  scale_color_manual(values=new_pallet_4) +
  #scale_color_viridis(begin = 0.2, end = 0.8, discrete = T, option = "A") +
  ylab("") +
  xlab("") +
  theme_bw() +
  NULL
...
#data.subset.by.rna.melt<- melt(data.subset.by.rna, id.vars = c("X.gene_id", "stage", "TOP"))
...
se2 <- ggplot(data=data.subset.by.rna.melt, aes((value), colour = TOP_plusC)) +
  ...
  scale_x_log10() + 
  coord_cartesian(xlim=c(0.1,10)) + 
  #scale_color_manual(values=pallet_2) +
...
# Save the legend
#+++++++++++++++++++++++
legend_se1 <- get_legend(se1)
legend_se2 <- get_legend(se2)

se1 <- se1 + theme(legend.position="none") +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                                                    panel.background = element_blank(), axis.line = element_line(colour = "black"))
se2 <- se2 + theme(legend.position="none") +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                                                    panel.background = element_blank(), axis.line = element_line(colour = "black"))

#se_plots <- grid.arrange(se1, se2, legend_se1, legend_se2, ncol=2, nrow=2)

#ggsave("/export/valenfs/projects/Hakon/RCP_SEQ/plots/new_IR_plots/cdf_plots/ecdf_MFE_50nt_leaders_all_stages_combined_plus.pdf", se_plots, height=150, width=250, units = 'mm')
#ggsave("/export/valenfs/projects/Hakon/RCP_SEQ/plots/new_IR_plots/cdf_plots/ecdf_MFE_50nt_leaders_all_stages_combined_plus.png", se_plots, height=150, width=250, units = 'mm')



###################################################################################################
# print densities for 1st, top, structure vs TE (seperatly) USING LSU FOR TE!!!
###################################################################################################
# NEW RNA seq
combined.dat.ranked.filter <- combined.dat.ranked %>% filter(complete_CDS_totalRNA_FPKM_new >=10, overlapping_gene==FALSE, leader_potentially_overlaps_upstream_gene==FALSE, gene_potentially_overlaps_downstream_leader==FALSE, gene_overlaps_non_coding_transcript==FALSE, TSS_start_sequence!="N", histone==FALSE)
...
#te2 <- ggplot(data=data.subset.by.rna.melt, aes((value), colour = TOP)) +
te2 <- ggplot(data=data.subset.by.rna.melt, aes((value), colour = TOP_plusC)) +
  stat_ecdf() +
  #scale_x_log10(limits=c(0.03,6)) +
  scale_x_log10() + 
  coord_cartesian(xlim=c(0.1,10)) + 
  #scale_color_manual(values=pallet_2) +
  scale_color_manual(values=new_pallet_3) +
...
# Save the legend
#+++++++++++++++++++++++
legend_te1 <- get_legend(te1)
legend_te2 <- get_legend(te2)


te1 <- te1 + theme(legend.position="none") +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                                                    panel.background = element_blank(), axis.line = element_line(colour = "black"))
te2 <- te2 + theme(legend.position="none") +  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                                                    panel.background = element_blank(), axis.line = element_line(colour = "black"))
#te_plots <- grid.arrange(te1, te2, legend_te1, legend_te2, ncol=2, nrow=2)

tl <- theme(legend.position = c(0.3, 0.67), legend.background=element_blank())
...
combse <- grid.arrange(se1 + tl + tit, se2 + tlb + titb, nrow = 1, bottom = "log10(Scanning efficiency)")
combte <- grid.arrange(te1 + tl + tit, te2 + tlb + titb, nrow = 1, bottom = "log10(Translational efficiency)")
comb <- grid.arrange(combse, combte, nrow = 2, left = "Cumulative fraction") 
#ggsave("/export/valenfs/projects/Hakon/RCP_SEQ/plots/new_IR_plots/cdf_plots/ecdf_MFE_50nt_leaders_all_stages_combined_TE_plus.pdf", te_plots, height=150, width=250, units = 'mm')
#ggsave("/export/valenfs/projects/Hakon/RCP_SEQ/plots/new_IR_plots/cdf_plots/ecdf_MFE_50nt_leaders_all_stages_combined_TE_plus.png", te_plots, height=150, width=250, units = 'mm')
# ggslackR(plot = se_plots)
# ggslackR(plot = te_plots)
