############ Threading ladders 
# INFO:
# These plots are for RCP-håkon-update
# And for RCP-seq paper

# Set directories
source("/export/valenfs/projects/uORFome/RCode1/ORFikPipeline.R")
mainFolder <- "/export/valenfs/projects/Hakon"
setwd(p(mainFolder, "/AdamVienna/"))
plotFolder <- p(getwd(), "/plots/new_plots/")
...
tx <- loadRegions(txdb, parts = c("tx", "leaders"))
...
reads5SSUAll <- ORFik:::convertToOneBasedRanges(readsSSUAll, method = "5prime", addSizeColumn = TRUE, addScoreColumn = TRUE)
reads3SSUAll <- ORFik:::convertToOneBasedRanges(readsSSUAll, method = "3prime", addSizeColumn = TRUE, addScoreColumn = TRUE)
...
readsSSU150NT <- ORFik:::readBam(SSUnt150Path, tx)
...
windowsOnetest <- removeBadTxByRegion(windowsOne, reads5SSUAll, upstream = 250, downstream = 300, 20, min_cutoff = 200)
...
gg <- windowCoveragePlot(rbindlist(list(hitMapTSS, hitMapTIS)), scoring = "sum", type = "relation to start codon (nt)")
