...
getLeadersFromCage <- function(nCageList){
  export.list <- c("cageFolder","cageFiles", "getRelativePathName", "regionUORFsFolder",
                   "getLeaders", "p", "dataFolder", "getGTF", "getRelativePathName",
...
getUorfsFromLeaders <- function(folder = regionUORFsFolder){
  ...
  export.list <- c("uorfFolder","folder","leadersList","dataFolder","getCDS","removeORFsWithStartInsideCDS",
    "removeORFsWithSameStartAsCDS","removeORFsWithSameStopAsCDS","sortPerGroup","removeORFsWithinCDS",
    "findMapORFs","faiName","getFasta","getSequencesFromFasta","FaFile", "extractTranscriptSeqs",
    "stopSites", "groupGRangesBy", "getUnfilteredUORFs", "filterORFs", "p")
...
getRNAFpkms <- function(){
  ...
  nrnaList <- nrow(getRiboRNAInfoTable())
  # all rfp features
  
  rnaFPKMs <- foreach(i=1:nrnaList, .combine = 'cbind', .export = c("getCageTx"),
                      .packages = c("GenomicFeatures", "ORFik")) %dopar% {
    SRR <- getRiboRNAInfoTable()$rna[i]
